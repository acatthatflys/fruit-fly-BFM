...
class CEMConfig:
    ...
    freeze_every: int = 3
    verbose: bool = True
    # scenario distribution and curriculum — gunnery-focused defaults, even easier early
    easy_frac: float = 0.4
    ...
    curriculum: List[Tuple[float, float, float]] = field(default_factory=lambda: [
        (0.0, 15.0, 20.0),   # first 15% gens: 15-20s (acquisition & pursuit)
        (0.15, 15.0, 20.0),  # gen 1-2 still easy (100% easy mix below)
        (0.3, 30.0, 45.0),   # next: 30-45s (skills emerge)
        (0.7, 60.0, 90.0),   # last 30%: 60-90s (maintain advantage, recovery)
    ])
    # scenario mix curriculum: (progress_start, easy, defensive, random)
    # Even easier early: 100% easy gen1-2, 80/5/15 gen3-6, then ramp
    scenario_mix_curriculum: List[Tuple[float, float, float, float]] = field(default_factory=lambda: [
        (0.0, 1.0, 0.0, 0.0),   # gen 1-2: 100% easy — learn to track vs cooperative
        (0.15, 0.8, 0.05, 0.15), # gen 3-6: 80% easy, 5% defensive, 15% random
        (0.4, 0.4, 0.3, 0.3),   # middle: 40/30/30 balanced
...
class BalancedTrainingSpace:
    ...
    def _sample_easy(self, rng: random.Random, tag: str) -> Scenario:
        # gunnery-focused final: even easier — 50% ultra-easy 250-350m ±5° (known-good gets 4 hits),
        # 25% super-easy 250-500m ±15°, 25% easy 250-900m ±30°. Gives 1° gradient early.
        # Prioritizes evals: thousands of ultra-easy fights before harder.
        r = rng.random()
        if r < 0.5:
            # ultra easy: 250-350m, ±5° — known-good gets 4 hits vs level 300m stern
            return Scenario(range_m=rng.uniform(250.0, 350.0),
                            taa_deg=rng.uniform(-5.0, 5.0),
                            nose_offset_deg=rng.uniform(-5.0, 5.0),
                            dz_m=rng.uniform(-30.0, 30.0),
                            alt_m=rng.uniform(4000.0, 7000.0),
                            v_a=rng.uniform(240.0, 300.0),
                            v_t=rng.uniform(220.0, 280.0),
                            phase_deg=rng.uniform(0.0, 360.0),
                            seed=rng.randrange(1 << 30), tag=tag + "_gunnery_ultra")
        elif r < 0.75:
            # super easy: 250-500m, ±15°
