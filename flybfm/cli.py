...
def _bench_time_per_fight(n_fights: int = 8, max_time_s: float = 60.0, decision_dt: float = 0.1,
                          easy_frac: float = 0.4, defensive_frac: float = 0.3, random_frac: float = 0.3) -> dict:
    """Quick benchmark using actual curriculum mix (suggestion 6).

    Previously Random vs level, max_time 20s, fully random starts -> underestimates
    late curriculum (60-90s vs jinker/bnz). Now uses BalancedTrainingSpace with
    easy/defensive/random mix and samples opponents from pool, max_time 60s default.
    """
    from .train.policy import RandomPolicy
    from .train.cem import BalancedTrainingSpace
    import time as _time
    space = BalancedTrainingSpace(easy_frac=easy_frac, defensive_frac=defensive_frac,
                                  random_frac=random_frac, seed=0)
    rng = random.Random(0)
    env_cfg = EnvConfig(max_time_s=max_time_s, decision_dt=decision_dt)
    rc = RewardConfig()
    blue = RandomPolicy(seed=0)
    # mix of opponents like real training
    opp_names = ["level", "nose-on", "guns", "lag", "jinker", "bnz"]
    times = []
    for i in range(n_fights):
        sc = space.sample(rng, tag="bench")
        red_name = rng.choice(opp_names)
        red = POOL_BY_NAME[red_name](seed=1+i)
        t0 = _time.time()
...
def cmd_train(args) -> int:
    ...
    # n_params now 92 linear (22*4+4) or 460 poly (115*4) — compute from proto
    n_params = {"features": 92, "brain": 10}.get(args.policy, 92)
    if args.policy == "brain":
        ...
        if resume:
            pass
        elif args.init == "known-good":
            from .train.imitation import known_good_linear_policy, known_good_poly_policy, clone_report
            print("warm start: known-good linear roll=-1*az_coarse-1.5*az_fine pull=+1*el_coarse+1.5*el_fine (gets hits)")
            teacher = known_good_poly_policy(seed=args.seed) if basis == "poly" else known_good_linear_policy(seed=args.seed)
            try:
                print("  " + clone_report(teacher, seed=args.seed + 1))
            except Exception as e:
                print(f"  clone report failed: {e}")
    ...
    if cfg.use_curriculum:
        print(f"curriculum: episode 15-20s→30-45s→60-90s, scenario mix 100/0/0→80/5/15→40/30/30→20/40/40, opponent level→all")
        print(f"  reward: tracking lead 1° w12 + 30° w2 inverse-quad, hit 10 kill 100, good trigger +2.0 bad -0.01, good_solution +0.5 in_wez +0.05, points 0.05, obs 22d lead az/el separate + tanh fine")
    else:
    ...
    # quick bench for estimated time — now uses curriculum mix + 60s (suggestion 6)
    try:
        bench = _bench_time_per_fight(n_fights=4, max_time_s=60.0, decision_dt=cfg.decision_dt,
                                      easy_frac=cfg.easy_frac, defensive_frac=cfg.defensive_frac,
                                      random_frac=cfg.random_frac)
        est_s = total_fights * bench["avg_fight_s"]
        # curriculum makes early fights shorter (15-20s) so ~0.75x avg
        est_s *= 0.75
        print(f"bench: avg fight {bench['avg_fight_s']*1000:.0f}ms ({bench['fights_per_second']:.1f} fights/s) on {bench['cpu_count']} CPUs, mix {cfg.easy_frac:.1f}/{cfg.defensive_frac:.1f}/{cfg.random_frac:.1f} 60s")
...
def _get_bench():
    ...
    if _bench_cache is None or now - _bench_time > 30:
        try:
            _bench_cache = _bench_time_per_fight(n_fights=6, max_time_s=60.0, decision_dt=0.1,
                                                 easy_frac=0.4, defensive_frac=0.3, random_frac=0.3)
            _bench_time = now
        except Exception as e:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    t.add_argument("--generations", type=int, default=16,
                   help="number of generations (default 16, ~8192 fights with pop64×8)")
    t.add_argument("--population", type=int, default=64,
                   help="candidates per generation (default 64, suggestion 7)")
    ...
    t.add_argument("--init", default="known-good", choices=["known-good", "imitation", "random"],
                   help="init: known-good linear roll=-2*lead_az (default, gets hits), imitation, random")
    t.add_argument("--basis", default="linear", choices=["linear", "poly"],
                   help="action basis; linear 92 params now best (suggestion 7), poly 460")
    ...
    t.add_argument("--no-scenario-mix-curriculum", action="store_true",
                   help="disable scenario mix curriculum 100/0/0→80/5/15→40/30/30→20/40/40")
